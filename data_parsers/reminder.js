...
exports.setupRemindersRunning = (reminders, bot, callback) => {
    ...
    console.log("setting up " + reminders.length + " reminders running.");
    ...
    for(let i = 0; i < reminders.length; i++) {
        ...
        getCallbackAtTime(reminders[i].whenTime, function() {
            console.log("Time to send reminder!");
...
exports.setupReminderRunning = (reminder, bot, callback) => {
    console.log("setting up reminder running.");
    let extras = {parse_mode: 'HTML'};
...
exports.saveRemindersJSON = (json_data, path) => {
    fs.writeFile(path, JSON.stringify(json_data), function(err, data) {
        if (err) throw err;
        console.log('Saved reminders to: ' + path);
    })
}

function getCallbackAtTime(whenTime, callback) {
    var now = new Date();
	var millisTill10 = whenTime.getTime() - now.getTime();
	if (millisTill10 < 0) {
...
function parseInputText(input_text, callback, errorCallback) {
  ...
  if (text.length > 2) {
    ...
    if(timeString.length == 2) {
        console.log("can be splitted with by ':'");
        ...
        if(when.endsWith('h')) {
            ...
            console.log(timeAmount + " = min");
            whentime2 = new Date();
            whentime2.setMinutes(whentime2.getMinutes()+timeAmount)
        } else {
            console.log("doesn't split by ':', but isn't mins and hours!");
        }
    }
    ...
    if(dateString == 'today') {
        ...
        console.log("dateStringParts: ", dateStringParts);
        if(dateStringParts.length >= 2 &&
           dateStringParts.length <= 3) {
            ...
            console.log("we assume it's today");
