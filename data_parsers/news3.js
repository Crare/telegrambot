...
news_by_province = [
  { province: "Etelä-Karjala", url: "18-141372", shortcode: "EK" },
...
exports.getHelpMessage = (callback) => {
  let output = "/n /news is a command to get news data from YLE. \r\n";
...
exports.getProvinces = (callback) => {
  let output = "*Here is a list of provinces(maakunnat) with shortcodes in Finland:*\r\n";
  for (i = 0; i < news_by_province.length; i++) {
...
exports.getYleNews = (province, lang, newsAmount, callback) => {
  ...
  if (lang == "en" ||
    ...
    if (req.url != undefined) {
      ...
      const output = "Sorry, couldn't get news-data.";
      callback(output);
    }

  } else {
    const output = "Possible languages are 'en', 'fi', 'ru', 'sa' and 'simplefi'.";
    callback(output);
  }
};
